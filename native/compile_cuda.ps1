# Build voxel_fdtd_cuda.dll (MinGW host; device code is NVRTC at runtime)
...
& $gpp -O3 -std=c++17 -shared -o voxel_fdtd_cuda.dll voxel_fdtd_cuda.cpp
if ($LASTEXITCODE -ne 0) { throw "cuda host compile failed" }
Write-Host "built $here\voxel_fdtd_cuda.dll"
...
if (-not (Test-Path "$here\fdtd_kernels.cu")) { throw "fdtd_kernels.cu missing" }
Write-Host "ok (kernels compiled at runtime by NVRTC)"
